...
export function DashboardSidebar() {
  ...
  const { patients, currentPatientId, patientWorkspaceOpen } = usePatientStore();
    ...
    <aside className="hidden w-72 shrink-0 border-r bg-card/80 backdrop-blur lg:flex lg:flex-col">
          ...
          <div className="mt-4 space-y-2">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">DOB {patient.dob}</p>
              </div>
            </Link>
          </div>
        ) : null}
      ...
      <div className="m-4 rounded-2xl border bg-secondary/70 p-4">
        ...
        <p className="mt-2 text-xs leading-5 text-muted-foreground">
          Track security posture, risky inboxes, device readiness, and practical remediation tasks.
        </p>
      </div>
    </aside>
  );
